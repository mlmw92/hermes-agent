    ...
    *,
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            # Support HERMES_FORCE_AUTO_START_PROFILES env var — comma-separated
            # list of profile names that should always auto-start on container boot,
            # regardless of their prior gateway state. This works around the deadlock
            # where a named profile's gateway writes "stopped" on shutdown, then the
            # reconciler sees "stopped" and refuses to auto-start it on next boot.
            force_profiles = os.environ.get("HERMES_FORCE_AUTO_START_PROFILES", "").strip()
            if force_profiles and entry.name in [p.strip() for p in force_profiles.split(",")]:
                should_start = True
...
def main() -> int:
    ...
    # Load .env so HERMES_FORCE_AUTO_START_PROFILES (and other env vars
    # the user set there) are visible to os.environ.get(). The s6
    # with-contenv shebang only populates /run/s6/container_environment/
    # — it does NOT source .env files, so without this explicit load
    # the reconciler would miss env vars that only exist in .env.
    try:
        from hermes_cli.env_loader import load_hermes_dotenv
        load_hermes_dotenv(hermes_home=hermes_home)
    except Exception:
        pass  # best-effort — don't block container boot
